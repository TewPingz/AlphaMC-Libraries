...
public class Cuboid implements Iterable<Location> {
    ...
    public List<Location> getBorder() {
        ...
        for (int x = minX; x <= maxX; ++x) {
            for (int y = minY; y <= maxY; ++y) {
                ...
                blocks.add(minLoc.getBlock().getLocation());
                blocks.add(maxLoc.getBlock().getLocation());
            }
        }
        for (int y = minY; y <= maxY; ++y) {
    ...
    public List<Block> getBlocks() {
        Iterator<Block> blockI = this.blockIterator();
    ...
    public boolean isNearby(Cuboid toCompare, int distance) {
        Location centerOne = this.getCenter();
        Location centerTwo = toCompare.getCenter();

        Vector vectorDistance = centerOne.toVector().subtract(centerTwo.toVector());
        int xDistance = Math.abs(vectorDistance.getBlockX());
        int zDistance = Math.abs(vectorDistance.getBlockZ());

        xDistance -= this.getSizeX() / 2;
        zDistance -= this.getSizeZ() / 2;

        xDistance -= toCompare.getSizeX() / 2;
        zDistance -= toCompare.getSizeZ() / 2;
