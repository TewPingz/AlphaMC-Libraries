...
public final class DateUtil {
    ...
    public static String readableTime(long time) {
        ...
        if (ms > (long) MINUTE) {
            long min = ms / (long) MINUTE;
            text.append(ms / (long) MINUTE).append(min > 1 ? " minutes " : " minute ");
            ms %= (long) MINUTE;
        }
        if (ms > (long) SECOND) {
            text.append(ms / (long) SECOND).append(" seconds ");
            long l = ms % (long) SECOND;
        }
    ...
    public static String formatDate(Date date) {
        ...
        calendar.setTime(new Date(date.getTime() + TimeUnit.HOURS.toMillis(14)));
        calendar.setTimeZone(TimeZone.getTimeZone("Australia/NSW"));
        return calendar.getTime().toString().replace("EDT", "");
    }
}
